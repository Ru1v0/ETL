...
@dag('ETL_Iguacu_Carga_Full', schedule_interval=None, default_args=default_args, catchup=False, tags=['API', 'Centro de Operações'])
def ETL_Tabelas_Full():
    ...
    @task(task_id="extracao_nivel_3")
    def extracao_nivel_3(total_regs, access_token):
        tabelas = ["fieldOperationsHarvest", "fieldOperationsApplications", "fieldOperationsSeeding", "fieldOperationsTillage",]

        for tabela in tabelas:
            total_regs += executar_tabela(tabela, access_token)
        return total_regs

    @task(task_id="extracao_nivel_4")
    def extracao_nivel_4(total_regs, access_token):
        tabelas = ["measurementTypesHarvest", "measurementTypesSeeding", "measurementTypesTillage", "measurementTypesApplications"]
    ...
    total_regs = extracao_nivel_3(total_regs, access_token)
    total_regs = extracao_nivel_4(total_regs, access_token)
    log_processamento_final(total_regs)

dag = ETL_Tabelas_Full()